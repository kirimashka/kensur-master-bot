...
SKLAD_INSTRUCTION = [
    ...
    "\n"
    "Это приложение внутри этого бота. В нём видно, какие комплекты и комплектующие KENSUR у вас на руках, и одной кнопкой отмечается, что вы поставили, продали или вернули. Устанавливать ничего не нужно.\n"
    "\n"
    "Главное правило: бот — про деньги за вашу работу по заявкам, Склад — про товар. Каждое событие отмечается один раз и в одном месте (см. пункт 5).\n"
    "\n"
    "1️⃣ Как зайти\n"
    ...
    "\n"
    "2️⃣ Первое дело: сверить остаток и подтвердить его фото\n"
    "Приложение должно показывать ровно то, что реально лежит у вас сейчас — ни больше, ни меньше. Пока остаток не сверен, «Поставил», «Продал» и «Счёт по СБП» не сработают: приложению нечего списывать.\n"
    "• Получите от Кирилла файл с номерами для вашего города.\n"
    "• Напишите номер маркером на каждой коробке комплекта (сбоку и на крышке). Какая коробка получит какой номер — неважно, главное, чтобы модель совпадала. Если на коробке уже есть наклейка с QR от склада, номер писать не нужно.\n"
    "• Сфотографируйте всё, что у вас лежит: коробки так, чтобы читались номера, и комплектующие (ключи, корпуса, планки, подложки, шлюзы). Фото подтверждают, что товар действительно у вас на руках. Без фото заявка не принимается.\n"
    "• В приложении: «Ещё» → «Получил товар». Выберите свой город. Приложение покажет, что числится за городом: нажмите на номера комплектов, которые у вас есть, поправьте количество комплектующих, добавьте фото, нажмите «Отправить заявку». Чего нет в списке — добавьте ниже.\n"
    "• Чего нет на руках — не отмечайте, даже если оно числится за вашим городом. Напишите об этом Кириллу.\n"
    ...
    "Дальше остаток меняется только кнопками, руками его никто не пересчитывает. Разошёлся с реальностью — сразу напишите Кириллу, не «подгоняйте» его через «Поставил» или «Утеря».",
    ...
    "🧾 Счёт клиенту по СБП — самый простой способ продать: выберите комплект, цену и телефон или почту клиента (туда придёт чек). Покажите клиенту QR, он платит на счёт компании. Деньги пришли — продажа записана, комплект списан, вам подтверждение в бот. Переводить ничего не нужно.\n"
    "💰 Продал — если клиент заплатил вам напрямую наличными или переводом. Укажите цену и способ оплаты; закупочную стоимость переводите Кириллу, как договорились.\n"
    ...
    "\n"
    "4️⃣ Счёт по СБП: за что можно, за что нельзя\n"
    "• Только за комплекты умных замков KENSUR и комплектующие к ним (ключи ic-key, корпус замка, торцевая планка, подложка под ручку, Wi-Fi шлюз, цилиндр для умного замка). Деньги идут на счёт компании, клиенту автоматически уходит чек на товар.\n"
    "• Свою работу по установке в счёт не включайте — установку клиент оплачивает вам напрямую, как вы с ним договорились.\n"
    "• Нельзя выставлять счёт за чужой товар и за то, чего нет на вашем складе в приложении.\n"
    "• Телефон или почта клиента обязательны: без них чек не уйдёт и счёт не создастся.\n"
    "\n"
    "5️⃣ Заявки от KENSUR: как отчитываться\n"
    "• Заявка от Кирилла (клиент купил замок с установкой, есть гарантийный талон, за работу платит Кирилл): отчёт как раньше — в боте кнопкой «📸 Новая установка»: адрес, до 5 фото (снаружи, изнутри, с торца, ответная часть и заполненный гарантийный талон), доп. расходы. Кирилл проставит сумму и отметит оплату, вы подтвердите получение кнопкой. В Складе ничего не отмечайте: замок клиента не с вашего склада.\n"
    "• На заявку поставили комплект со своего склада: отчёт в боте как выше и дополнительно «🔧 Поставил» в Складе, чтобы комплект списался. Это две разные вещи: бот — оплата за работу, Склад — движение товара.\n"
